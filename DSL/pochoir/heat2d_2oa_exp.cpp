...
int main(int argc, char * argv[])
{
	...
	Pochoir_Array<double,N_RANK> a(N_SIZE,M_SIZE);
    Pochoir<N_RANK> heat_2D(heat_shape_2D);

    Pochoir_Kernel_2D(heat_2D_fn, t, i,j)
	   a(t+1, i, j) = 0.125 * (a(t, i+1, j) + a(t, i-1,j) + a(t,i,j-1) + a(t,i,j+1) - 4.0*a(t,i,j)) + a(t,i,j);
    Pochoir_Kernel_End
  ...
  t2 = seconds();
    ...
    double nflops = (double) (N_SIZE - 2) * (double) (M_SIZE - 2) * T_SIZE * 5;
  cout << "FLOPs in stencil code: " << nflops << endl;
	cout << "Time spent in stencil coe: " << t2-t1 << "ms" << endl;
