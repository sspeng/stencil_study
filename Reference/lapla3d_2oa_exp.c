/*
 * Reference for laplacian three-dimensional 2nd order accurate (spatial) explicit method
 *
 * Original equation: U_xx + U_yy + U_zz = 0
 * Solved by: u(t+1,x,y,z) = 
 *      (u(t,x,y,z) 
 *      - u(t,x-1,y,z) - u(t,x+1,y,z) 
 *      - u(t,x,y-1,z) - u(t,x,y+1,z)
 *      - u(t,x,y,z-1) - u(t,x,y,z+1))/6.0
 *
...
int main(int argc, char** argv)
{
      ...
      for (y = 1; y < y_max - 1; y++)
      {
        for (z = 1; z < z_max - 1; z++)
        {
          u_0_1[IDX(x,y,z)] = (u_0_0[IDX(x,y,z)] 
              ...
              - u_0_0[IDX(x,y,z-1)] - u_0_0[IDX(x,y,z+1)])/6.0;
        }
      }
    }
    ...
    /* print statistics */    
    nFlops = (double) (x_max-2) * (double) (y_max-2) * (double) (z_max-2) * T_MAX * 7.0;
