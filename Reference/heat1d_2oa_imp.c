...
int main(int argc, char** argv)
{
    ...
    double diff = 1; // Value here doesn't matter because it is reset before used
    long n_iters = 0;
    /* do the calculation */ 
	t1 = seconds();
	while (eps <= diff)
	{
    diff = 0; // Find the largest difference
		#pragma omp parallel for private(z,y,x)
    for (x = 1; x < x_max - 1; x++)
    {
      u_0_1[IDX(x)] = u_0_0[IDX(x)] +
            0.125 * (u_0_0[IDX(x+1)] - 2*u_0_0[x] + u_0_0[IDX(x-1)]);

      double tmp_diff = fabs(u_0_1[x] - u_0_0[x]);
    ...
    /* print statistics */    
    nFlops = (double) (x_max-2) * n_iters * 5.0;
